...
for i in xrange(0,len(data)):
	for j in xrange(1,len(data[i])):
		try:
			...
			del data[i] 
users = {dat[0]: [CMAB(N_strategies, steps = steps), [dat[1:][j:j+steps] for j in xrange(0, len(dat[1:]), steps)], [], []] for dat in data}
...
for j in xrange(N_Random_users):
	...
	users['rand_user_' + str(j)] = [CMAB(N_strategies, steps = steps), temp, [], []]
